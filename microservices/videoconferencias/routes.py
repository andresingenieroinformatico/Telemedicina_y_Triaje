...
# ─── POST /salas/join (COMPATIBILIDAD CON FRONTEND) ───────────────────────────
@video_bp.route("/salas/join", methods=["POST"])
def unirse_sala_frontend():
    """
    Endpoint de compatibilidad para el frontend de React.
    Busca la sala por nombre_sala, si no existe la crea, y luego genera el link.
    Body JSON: nombre_sala, usuario_id, rol, nombre_usuario
    """
    data = request.get_json()
    if not data:
        return _err("Se requiere cuerpo JSON.", 400)
    ...
    # Buscar sala o crearla si no existe
    sala = Sala.query.filter_by(nombre=nombre_sala).first()
    if not sala:
        sala = Sala(nombre=nombre_sala, url_sala="__pending__", capacidad_max=10)
        db.session.add(sala)
        db.session.flush()
        room_name = generate_room_name(sala.id, sala.nombre)
        sala.jitsi_room_name = room_name
        sala.url_sala = generate_jitsi_url(room_name)
        db.session.commit()

    if sala.estado != "activa":
        return _err("La sala no está activa.", 409)
